...
public class JwtTokenProvider {
    ...
    public String createToken(TokenDTO dto, JwtTokenType jwtTokenType) {
        ...
        if (jwtTokenType == JwtTokenType.REFRESH_TOKEN) { // Access Token이라면
    ...
    /**
      * 쿠키에 저장 및 HTTP 인증 헤더에 저장된 JWT 토큰 ( Access Token, Refresh Token ) 가져오기
    ...
    public JwtStatus validateToken(String token) {
        ...
        if (!token.isEmpty()) {
            try {
                ...
                return JwtStatus.EXPIRED; // 혹시 몰라서 Exception으로 한 번 더 체크 기간 만료
